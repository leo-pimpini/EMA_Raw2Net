# ================================
# LMM for Self_Control (continuous)
...
stopifnot(all(c(id_col, time_col, "Self_Control") %in% names(dat)))
...
dat <- dat %>%
  ...
  mutate(
    ...
    Self_Control = as.numeric(Self_Control)
  ) %>%
...
# ---- Descriptives ----
vars_to_sum <- c("Self_Control","Time")
...
m_SC <- mean(dat$Self_Control, na.rm = TRUE)
v_SC <- var(dat$Self_Control,  na.rm = TRUE)
cat("Mean(Self_Control) =", m_SC, " | Var(Self_Control) =", v_SC, "\n")

# Distribution plot
ggplot(dat, aes(x = Self_Control)) +
  geom_histogram(binwidth = diff(range(dat$Self_Control, na.rm = TRUE))/30, color = "white") +
  labs(title = "Distribution of Self_Control",
       x = "Self_Control", y = "Frequency") +
  theme_minimal()

# ---- Model (Gaussian LMM) ----
# Mirror your structure with a random intercept & slope for Time
form_lmm <- as.formula("Self_Control ~ Time * Group + (Time | ID)")
...
ggplot(dat_sub, aes(x = Time, y = Self_Control, color = Group)) +
  ...
  labs(
    title = "Self-control (n = 20 per group)",
    x = "Time (days)",
    y = "Self-control (VAS)"
  ) +
  theme_minimal()
...
# Robust column mapping (different emmeans versions name CI columns differently)
...
ggplot(pred_df, aes(x = Time, y = emmean, color = Group, fill = Group)) +
  ...
  labs(
    title = "Self-control (all participants)",
    x = "Time (days)",
    y = "Self-control (VAS)"
  ) +
  theme_minimal()
