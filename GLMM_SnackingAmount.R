# ================================
# GLMM for Snacking Amount (SA; count)
...
stopifnot(all(c(id_col, time_col, "SA") %in% names(dat)))
...
m_SA <- mean(dat$SA, na.rm = TRUE)
v_SA <- var(dat$SA,  na.rm = TRUE)
cat("Mean(SA) =", m_SA, " | Var(SA) =", v_SA, "\n")

# Distribution plot
ggplot(dat, aes(x = SA)) +
  geom_histogram(binwidth = 1, color = "white") +
  labs(title = "Distribution of Snacking Amount (SA)",
       x = "Count of snacks", y = "Frequency") +
  theme_minimal()
...
form_glmm <- as.formula("SA ~ Time * Group + (Time | ID)")
...
ggplot(dat_sub, aes(x = Time, y = SA, color = Group)) +
  ...
  labs(
    title = "Snacking amount (n= 20 per group)",
    x = "Time (days)",
    y = "Snacking amount (portions)"
  ) +
  theme_minimal()
...
#  labs(
#    title = paste0("Snacking Amount ((SA) Over Time\nModel: )all ", best_name),
#    x = "Time (days since 1st observation)",
#    y = "Predicted SA (count, response scale)"
#  ) +
#  theme_minimal()
...
ggplot(pred_df, aes(x = Time, y = emmean, color = Group, fill = Group)) +
  ...
  geom_line(linewidth = 1) +
  labs(
    title = "Snacking amount (all participants)",
    x = "Time (days)",
    y = "Snacking amount (portions)"
  ) +
  theme_minimal()
